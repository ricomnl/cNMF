...
class cNMF:
    ...
    def _initialize_dirs(self):
        if self.paths is None:
            ...
            self.paths = {
                ...
                "merged_usages": os.path.join(
                    self.output_dir,
                    self.name,
                    "cnmf_tmp",
                    self.name + ".usages.k_%d.merged.df.npz",
                ),
                ...
                "k_selection_plot_program": os.path.join(
                    self.output_dir, self.name, self.name + ".k_selection_program.png"
                ),
                "k_selection_plot_usages": os.path.join(
                    self.output_dir, self.name, self.name + ".k_selection_usages.png"
                ),
    ...
    def combine(self, components=None, skip_missing_files=False):
        ...
        for k in ks:
            self.combine_nmf(
                k, data_type="spectra", skip_missing_files=skip_missing_files
            )
            self.combine_nmf(
                k, data_type="usages", skip_missing_files=skip_missing_files
            )
    ...
    def get_norm_counts(
        ...
        if high_variance_genes_filter is None:
            ...
            if sp.issparse(tpm.X):
                (gene_counts_stats, _) = get_highvar_genes_sparse(
                    tpm.X, numgenes=num_highvar_genes
                )
            else:
                (gene_counts_stats, _) = get_highvar_genes(
    ...
    def _nmf(self, X, nmf_kwargs):
        ...
        """
        (usages, spectra, _) = non_negative_factorization(X, **nmf_kwargs)
    ...
    def factorize(
        ...
        for idx in jobs_for_this_worker:
            ...
            usages = pd.DataFrame(
                ...
                columns=np.arange(1, _nmf_kwargs["n_components"] + 1),
            )
            save_df_to_npz(
                spectra, self.paths["iter_spectra"] % (p["n_components"], p["iter"])
            )
            save_df_to_npz(
                usages, self.paths["iter_usages"] % (p["n_components"], p["iter"])
            )
    ...
    def combine_nmf(self, k, data_type="spectra", skip_missing_files=False):
        ...
        for _, p in run_params_subset.iterrows():
            current_file = self.paths["iter_%s" % data_type] % (
                p["n_components"],
                p["iter"],
            )
            if not os.path.exists(current_file):
                ...
                data = load_df_from_npz(current_file)
        ...
        if len(combined_data) > 0:
            if data_type == "spectra":
                ...
                raise ValueError("Unknown data_type: %s" % data_type)
            save_df_to_npz(combined_data, self.paths["merged_%s" % data_type] % k)
        else:
            print("No %s found for k=%d" % (data_type, k))
    ...
    @staticmethod
    def program_stability(merged_spectra, k):
        kmeans_model = KMeans(n_clusters=k, n_init=10, random_state=1)
        kmeans_model.fit(merged_spectra)
        kmeans_cluster_labels = pd.Series(
            kmeans_model.labels_ + 1, index=merged_spectra.index
        )

        stability = silhouette_score(
            merged_spectra.values, kmeans_cluster_labels, metric="euclidean"
        )
        return stability, kmeans_cluster_labels

    def k_stability_stats(self, k, density_threshold=0.5):
        ...
        program_stability, kmeans_cluster_labels = cNMF.program_stability(
            merged_spectra=l2_spectra, k=k
        )
        usage_stability = cNMF.usages_stability(merged_usages=merged_usages, k=k)
    ...
    def consensus(
        ...
        _, kmeans_cluster_labels = cNMF.program_stability(
            merged_spectra=l2_spectra, k=k
        )
        ...
        consensus_stats = self.k_stability_stats(
        ...
        if refit_usage:
            ...
            # in case one gene is divided by 0 it causes a NaN and the nmf won't work
            spectra_tpm_rf = spectra_tpm_rf.fillna(0.0)
    ...
    @staticmethod
    def plot_stability(stats, stability_type="program", save_dir=None, close_fig=False):
        fig = plt.figure(figsize=(6, 4))
        ax1 = fig.add_subplot(111)
        ax2 = ax1.twinx()

        ax1.plot(stats["k"], stats["%s_stability" % stability_type], "o-", color="b")
        ax1.set_ylabel(
            "%s Stability" % stability_type.capitalize(), color="b", fontsize=15
        )
...
def main():
    ...
    parser.add_argument(
        "--l1-ratio",
        type=float,
        help="[prepare] L1 Ratio applied to non-negative matrix factorization.",
        default=0.0,
    )
    parser.add_argument(
        "--beta-loss",
    ...
    parser.add_argument(
        "--max-iter", type=int, help="[prepare] Max iterations for NMF.", default=1000
    )
    parser.add_argument(
        "--densify",
        dest="densify",
